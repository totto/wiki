...
![KCP v0.18 — Signing the Map, Not the Territory: a JWS signature covers the manifest YAML, but not the files the units point to. v0.18 adds per-unit content hashes and origin evidence classes to close the gap.](../../assets/images/kcp-018-01-title.png)
...
![The T9 threat vector: a genuine knowledge.yaml (signature valid) in an attacker-controlled directory with a fabricated .git/config claiming a legitimate origin. Every RFC-0018 check passes. The unit paths resolve to attacker-authored files.](../../assets/images/kcp-018-03-threat-vector.png)
...
![The 4 steps of T9 execution: (1) public signed manifest, (2) attacker copies and fabricates, (3) renderer verifies — valid JWS, real key, origin in scope, (4) attacker content injected at highest trust placement.](../../assets/images/kcp-018-04-four-steps.png)
...
![Signing the Territory: the full picture — Fix 1 (per-unit content hashes, C11/C12), Fix 2 (origin evidence classes, C13), the B20 corroboration safeguard (C14), and layered security outcomes. Defense in depth: either mechanism alone degrades the attack; together they close it.](../../assets/images/kcp-018-00-overview.png)
