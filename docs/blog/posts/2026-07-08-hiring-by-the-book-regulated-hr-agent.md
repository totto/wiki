...
Deliberately, this scenario is **not** going into the kcp-agent repository. The reference agent stays vendor-neutral, Apache-2.0, runnable by anyone with `npx`. What belongs upstream from this exercise are the sharpened knives: the `not_for` lint rule, the fail-closed handling of unrecognized trust declarations, and — further out — precedence relations and the transparency log.
