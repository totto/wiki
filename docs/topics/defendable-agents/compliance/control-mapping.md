...
Read the table below with one question in mind for each row: *if I deleted the policy sentence, would the control still hold?* For a defendable agent the answer is yes. Audit logging is not a promise to log — it is an append-only writer with `fsync` on flush that the [governed session](/topics/defendable-agents/architecture/governance-harness/) cannot bypass. Reproducibility is not a claim that scores are consistent — it is a [deterministic planner](/topics/defendable-agents/decisions/reproducibility/) of pure functions where the same inputs always yield the same outputs. The control is the code path, and the evidence is what that path wrote down.
...
```json
{
  ...
  "temporal": {
    ...
    "signalDates": ["2026-03-04T00:00:00Z"]
  },
